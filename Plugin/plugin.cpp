...
namespace Plugin
{
    ...
    void RegisterPatchSteps()
    {
        ...
        //修正无脑跳单词的逻辑
        batch_matching::get_instance().register_step("0F B7 07 83 F8 7E 74 0A", 1, [](const std::vector<memory_pointer>& addresses)
            {
                injector::MakeNOP(addresses[0].i(), 6);
                injector::MakeCALL(addresses[0].i(), CFont::Epilog_922054);
            });

        batch_matching::get_instance().register_step("F3 0F 11 54 24 28 E8 ? ? ? ? 8B D8", 1, [](const std::vector<memory_pointer>& addresses)
            {
                injector::MakeCALL(addresses[0].i(6), CFont::Prolog_9224A5);
            });
